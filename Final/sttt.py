...
class GameView(object):
    ...
    def drawplayername(self, name, color):
        ...
        self.background.blit(self.robotocondensedL.render(name, True, color), (650,730))
        pass
    ...
    def drawgamearray(self,playerList, Model):
        ...
        for playerloop in playerList:
            for position in playerloop.positions:
                self.place_bug(playerloop, Model.sectorcenter(position), position) 
...
def stttmain(playerNames):
    ...
    while True:
        ...
        if mouseSector != (None,None):
            ...
            TTTView.place_bug(TTTModel.currentplayer,mouseSectorCenter, mouseSector)
            if mouseclick == 'Left':
